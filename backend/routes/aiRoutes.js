...
router.post('/scan-meal', auth, async (req, res) => {
    try {
        ...
        if (!base64Image) return res.status(400).json({ message: "No image" });

        // Initialize with explicit API version
        const genAI = new GoogleGenerativeAI(process.env.GEMINI_API_KEY);
        
        // Use the full model path to avoid 404 errors
        ...
        res.status(500).json({ message: "Model connection failed. Please try again." });
    }
});
